...
(işlev yazmanıza gerek yok)
Fenomenler dizisinde bir yazım hatası var 😱 7. sıradaki fenomen 'Jiustn Bieber' ın soyismi 'Biber' olarak yanlış yazılmış. Bu sorunu düzeltin ve çalışmanızı kontrol etmek için console.log() yapın.

fenomenler[6].profile = "Jiustn Bieber"
...
function indekseGoreFenomen(fenomenler,i) {
  let profil = fenomenler[i].profile;
  let result= i + ". indekste bulunan fenomen: " + profil;
  return result;
}
...
function fenomenEkle(fenomenler,number,profile,followers,post,platform) {
  let yeniArr = [...fenomenler];
  let newObj = {
     "number": 6,
      "profile": "Workintech",
      "followers": 10000000,
...
*/
const enFenomenler = (fenomenler) => {
  const yuzMilyonFenomenler = fenomenler.filter((fenomen) => fenomen.followers > 100000000);
  const yuzMilyonFenomenlerinAdlari = yuzMilyonFenomenler.map((fenomen) => fenomen.profile);
  return yuzMilyonFenomenlerinAdlari;
  };
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform){
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].platform === platform && fenomenler[i].posts !== "NA" && fenomenler[i].posts > maxPosts) {
      ...
      maxPostsFenomen = fenomenler[i].profile;
    }
  }
